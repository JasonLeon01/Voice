...
def main():
    ...
    model.load_state_dict(torch.load('d:\\Dataset\\denoise_model_epoch20.pth', map_location=device))
    model.eval()

    # 去噪并直接保存到根目录
    denoised, sr = denoise_one(result_path, model, device)
    denoised_path = "d:\\Dataset\\denoised.wav"
    torchaudio.save(denoised_path, denoised, sr)

    # 复制result.wav到根目录
    result_copy_path = "d:\\Dataset\\result.wav"
